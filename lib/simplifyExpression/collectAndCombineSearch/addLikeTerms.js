...
function evaluateCoefficientSum(node) {
  ...
  const symbolNode = node.cloneDeep().args[1];
  const childStatus = evaluateConstantSum(coefficientSum);

  // If coefficientSum evaluates to 1/-1 return only variable/-variable, e.g. x not 1*x.
  return removeOneOrMinusOneCoefficients(Node.Status.childChanged(node, childStatus, 0), childStatus, symbolNode);
}

// If coefficientSum evaluates to 1/-1 return only variable/-variable, e.g. x not 1*x.
function removeOneOrMinusOneCoefficients(status, childStatus, symbolNode){
  if (!Node.Type.isConstant(childStatus.newNode)){
    return status;
  }
  // If coefficientSum evaluates to 1 return only variable, e.g. x not 1*x.
  if (childStatus.newNode.value === '1'){
    status.newNode = symbolNode;
  }
  else if (childStatus.newNode.value === '-1'){
    status.newNode = Node.Creator.unaryMinus(symbolNode);
  }
  return status;
}
