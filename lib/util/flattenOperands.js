...
function flattenOperands(node, forceMultiplicationSimplification) {
  ...
  if (Node.Type.isConstant(node, true)) {
    ...
    if ('+-/*'.includes(node.op)) {
      ...
      return flattenSupportedOperation(node, parentOp, forceMultiplicationSimplification);
    }
...
function flattenSupportedOperation(node, parentOp, forceMultiplicationSimplification) {
  ...
  if (operands.length === 1) {
    ...
    if (node.op === '*') {
      node.implicit = forceMultiplicationSimplification || false;
    }
  }
  return node;
}
